...
async def on_raw_reaction_add(payload):
    ...
    if startup_active and startup_message and payload.message_id == startup_message.id:
        if payload.emoji.id == 1483008124024524820:
...
@bot.tree.command(
    name="startup",
    description="Start a convoy session and request reactions from members."
)
@app_commands.describe(reactions="Amount of reactions required before session release")
...
@bot.tree.command(
    name="link",
    description="Release the private server link for the convoy."
)
