...
TICKET_SUPPORT_VIEW_ROLE = 1474123995375992873
TICKET_STAFF_VIEW_ROLE = 1474121009656500225
TICKET_TRANSCRIPT_CHANNEL = 1471451529838858414
TICKET_STORAGE_FILE = "/mnt/disk/tickets.json"
TICKET_PANEL_FILE = "/mnt/disk/ticket_panel.json"

os.makedirs(os.path.dirname(TICKET_STORAGE_FILE), exist_ok=True)


def load_ticket_data():
    default = {"open_by_user": {}, "tickets": {}}
    ...
    try:
        with open(TICKET_STORAGE_FILE, "r") as f:
            ...
            if not isinstance(data, dict):
                return default
            open_by_user = data.get("open_by_user", {})
            tickets = data.get("tickets", {})
            if not isinstance(open_by_user, dict):
                open_by_user = {}
...
def save_ticket_data(data):
    with open(TICKET_STORAGE_FILE, "w") as f:
        json.dump(data, f, indent=4)


def load_ticket_panel():
    if not os.path.exists(TICKET_PANEL_FILE):
        return {}
    try:
...
def ticket_type_label(ticket_type: str):
    mapping = {
...
def ticket_channel_name(ticket_type: str, user_id: int):
    suffix = str(user_id)[-4:]
    if ticket_type == "general_support":
        return f"general-support-{suffix}"
    if ticket_type == "staff_report":
        return f"staff-report-{suffix}"
    return f"user-report-{suffix}"
...
class TicketTypeSelect(ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(label="General Support", value="general_support", description="Open a general support ticket"),
            discord.SelectOption(label="Staff Report", value="staff_report", description="Open a staff report ticket"),
            discord.SelectOption(label="User Report", value="user_report", description="Open a user report ticket"),
        ]
        super().__init__(
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        member = interaction.guild.get_member(interaction.user.id)
        if member is None:
            await interaction.response.send_message("Unable to verify your member profile. Please try again.", ephemeral=True)
            return
        data = load_ticket_data()

        user_map = data["open_by_user"].setdefault(str(interaction.user.id), {})
        existing_channel_id = user_map.get(ticket_type)
        if existing_channel_id:
            ...
            if existing_channel is not None:
                ...
                return
            user_map.pop(ticket_type, None)
            data["tickets"].pop(str(existing_channel_id), None)
        ...
        if ticket_type in ("general_support", "user_report"):
            support_role = interaction.guild.get_role(TICKET_SUPPORT_VIEW_ROLE)
            if support_role:
                overwrites[support_role] = discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True)
        elif ticket_type == "staff_report":
            staff_role = interaction.guild.get_role(TICKET_STAFF_VIEW_ROLE)
            if staff_role:
                overwrites[staff_role] = discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True)

        channel_name = ticket_channel_name(ticket_type, interaction.user.id)
        ...
        user_map[ticket_type] = ticket_channel.id
        save_ticket_data(data)

        embed = discord.Embed(
            title=f"{ticket_type_label(ticket_type)} Ticket",
            description=(
...
class TranscriptLinkView(ui.View):
...
def build_transcript_html(channel: discord.TextChannel, messages: List[discord.Message]):
    ...
    for message in reversed(messages):
        ...
        for attachment in message.attachments:
            attachment_lines.append(f'<li><a href="{html.escape(attachment.url)}">{html.escape(attachment.filename)}</a></li>')
        attachments_html = f"<ul>{''.join(attachment_lines)}</ul>" if attachment_lines else ""
        author = html.escape(f"{message.author} ({message.author.id})")
        rows.append(
            f"<div style='padding:10px;border-bottom:1px solid #ddd;'>"
            f"<div><strong>{author}</strong> - <em>{created}</em></div>"
            f"<div style='white-space:pre-wrap;margin-top:5px;'>{content}</div>"
            f"{attachments_html}"
            f"</div>"
        )

    body = "".join(rows) if rows else "<p>No messages were found in this ticket.</p>"
    return (
        "<!DOCTYPE html><html><head><meta charset='utf-8'><title>Ticket Transcript</title></head>"
        "<body style='font-family:Arial,sans-serif;background:#f8f9fa;color:#222;'>"
        f"<h2>Ticket Transcript - #{html.escape(channel.name)}</h2>"
        f"<p>Channel ID: {channel.id}</p>"
        f"{body}</body></html>"
    )
...
class TicketCloseView(ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: ui.Button):
        ...
        messages = [m async for m in channel.history(limit=None, oldest_first=False)]
        involved = sorted({m.author.id for m in messages})
        creator_id = ticket_info.get("creator_id")
        closer_id = interaction.user.id
        transcript_html = build_transcript_html(channel, messages)
        ...
        info_embed = discord.Embed(
            ...
            description=f"Ticket type: **{ticket_type_label(ticket_info.get('ticket_type', 'unknown'))}**",
            color=0x87CEFA
        )
        info_embed.add_field(name="Creator", value=f"<@{creator_id}> (`{creator_id}`)", inline=False)
        info_embed.add_field(name="Closed By", value=f"<@{closer_id}> (`{closer_id}`)", inline=False)
        info_embed.add_field(
            name="Users Involved",
            value=", ".join([f"<@{uid}>" for uid in involved]) if involved else "None",
            inline=False
        )
        info_embed.add_field(name="Ticket Channel", value=f"{channel.name} (`{channel.id}`)", inline=False)
        info_embed.set_footer(text="Greenville Mafia Corporation", icon_url=FOOTER_ICON)
        ...
        if ticket_type in user_map and user_map[ticket_type] == channel.id:
            user_map.pop(ticket_type, None)
        if not user_map and str(creator_id) in data["open_by_user"]:
            data["open_by_user"].pop(str(creator_id), None)
        data["tickets"].pop(str(channel.id), None)
        save_ticket_data(data)

        await interaction.followup.send("Ticket closed and transcript archived.", ephemeral=True)
...
async def ensure_ticket_panel_message():
    ...
    existing_id = panel_data.get("message_id")
    if existing_id:
        try:
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "Please select a ticket type from the dropdown below."
        ),
        color=0x87CEFA
    )
    embed.set_footer(text="Greenville Mafia Corporation", icon_url=FOOTER_ICON)
